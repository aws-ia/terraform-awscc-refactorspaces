variable "proxy_vpc_name" {
  type        = string
  description = "Name for Amazon VPC to host the link for AWS Refactor Spaces to access the monolithic application."
  default     = "vpc-application-example-proxy"
}

variable "proxy_vpc_cidr_block" {
  type        = string
  description = "A 24-bit IPv4 CIDR for Amazon VPC to host the link for AWS Refactor Spaces to access the monolithic application."
  default     = "192.168.10.0/24"
}

variable "environment_name" {
  type        = string
  description = "Name of AWS Migration Hub Refactor Spaces environment."
  default     = "Unistore-Dev"
}

variable "environment_description" {
  type        = string
  description = "Description for AWS Migration Hub Refactor Spaces environment."
  default     = "AWS Migration Hub Refactor Spaces environment for Unicorn Enterprises."
}

variable "application_name" {
  type        = string
  description = "Display name in AWS Migration Hub Refactor Spaces for the strangled application."
  default     = "Unistore"
}
...
variable "application_proxy_type" {
  type        = string
  description = "Accessibility of Amazon API Gateway deployed by AWS Migration Hub Refactor Spaces. `REGIONAL` APIs are publicly accessible in the current AWS Region. `PRIVATE` APIs are accessible only from VPCs. Default: REGIONAL."
  default     = "REGIONAL"
}

variable "display_name_monolith_server" {
  type        = string
  description = "Display name for AWS Migration Hub Refactor Spaces service for the monolithic application."
  default     = "legacy"
}

variable "description_monolith_server" {
  type        = string
  description = "Description for AWS Migration Hub Refactor Spaces service for the monolithic application."
...
variable "vpc_id_monolith" {
  type        = string
  description = "Amazon VPC ID of current monolithic application running (defined in `var.url_endpoint_monolith`)."
  default     = "http://legacy-unicorns.example.com/"
}
...
variable "display_name_lambda_microservice" {
  type        = string
  description = "Display name for Lambda-based microservice configured as AWS Migration Hub Refactor Spaces service to strangle a function of the monolith."
  default     = "Unistore-AddToCartService"
}

variable "description_lambda_microservice" {
  type        = string
  description = "Description for Lambda-based microservice configured as AWS Migration Hub Refactor Spaces service to strangle a function of the monolith."
